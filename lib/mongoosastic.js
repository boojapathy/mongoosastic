...
function Mongoosastic(schema, pluginOpts) {
  ...
  var bulkTimeout, bulkBuffer = [], esClient,
    ...
    parentFieldName = options && options.parent,
    routingFieldName = options && options.routing,
  ...
  schema.methods.index = function schemaIndex(inOpts, inCb) {
    var index, type, serialModel, parent, routing,
      cb = inCb,
      opts = inOpts;
    ...
    parent = opts.parent || parentFieldName;
    routing = opts.routing || routingFieldName;
    ...
    if (bulk) {
      ...
      if(parent) {
        objectToIndex['parent'] = serialModel[parent];
      }
      if(routing) {
       objectToIndex['routing'] = serialModel[routing]; 
      }
